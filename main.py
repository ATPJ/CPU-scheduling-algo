...
def main(number: int, algo: Algo, time_slice: Annotated[Optional[int], typer.Argument()] = 10,
    ...
    if context_switch:
        ctx_t = int(input("Give the time context switching: "))
        sch = Scheduler(algo, processes, qunt=time_slice, ctx_t=ctx_t, with_ctx=context_switch)
    else: sch = Scheduler(algo, processes, qunt=time_slice)
    sch.calculate()


@app.command(name='compare')
def compare(number: int, time_slice: Annotated[Optional[int], typer.Argument()] = 10,
            show_plot: bool = False, context_switch: bool = False):
    processes_info = []
    ...
    for algo in (Algo.fcfs, Algo.spn, Algo.hrrn, Algo.rr, Algo.srtf):
        ...
        if context_switch: sch = Scheduler(algo, processes, qunt=time_slice, ctx_t=ctx_t, with_ctx=context_switch)
        else: sch = Scheduler(algo, processes, qunt=time_slice)
